...
Bounded Context is principle of the strategy design of <<term-DDD,Domain-Driven Design>>. "A bounded context explicitly defines the context within which a <<term-domain-mode,domain model>> for a software system applies. Ideally, it would be preferable to have a single, unified model for all software systems in the same domain. While this is a noble goal, in reality it typically fragments into multiple models. It is useful to recognize this fact of life and work with it." (quote from Wikipedia)
